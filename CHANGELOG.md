...
- 群聊交互状态改为按聊天和用户共同隔离，避免成员之间相互覆盖搜索、浏览、下载及配置流程。
- 存储目录、文件和删除按钮现在校验当前存储根及目录；删除确认绑定到对应确认消息，避免旧按钮跨会话进入或误删文件。
- 配置更新改为先成功写盘再发布到内存，保存失败不再造成运行时配置与磁盘配置不一致；搜索源和代理切换也会正确报告保存错误。
- 缺少 `proxy` 节点时，修改代理地址或端口会自动建立完整配置；无效布尔值和端口会被拒绝。
- OpenList 启动检查改用需要鉴权的管理接口并验证 API 响应码，无效 Token 不再被误报为登录成功。
- Docker Compose 将 `config.yaml` 改为读写挂载，使机器人内的配置管理功能可以持久化修改。
- `log_level` 配置现在实际控制日志过滤级别；`/download` 命令入口补齐管理员鉴权。
- 新建目录名称会拒绝空值、路径分隔符、`.` 和 `..`，防止目录输入逃逸当前路径。
